...
<template>
    <dialog class="z-50 m-0 min-h-full min-w-full overflow-y-auto bg-transparent backdrop:bg-transparent" ref="dialog">
        <div class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" scroll-region>
            ...
            >
                <div v-show="show" class="mb-6 rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full sm:mx-auto" :class="[maxWidthClass, layout.isDarkTheme.value ? 'bg-gray-800' : 'bg-white']" @click.stop>
                    <slot v-if="showSlot"/>
                </div>
            </transition>
        </div>
